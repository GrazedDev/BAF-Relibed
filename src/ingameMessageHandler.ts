            ...
            if (title.toString().includes('Your Bids')) {
                ...
                for (let i = 0; i < window.slots.length; i++) {
                    ...
                    if (useCollectAll && slot?.type === 380 && name?.includes('Claim') && name?.includes('All')) {
                        ...
                        bot.currentWindow.requiresConfirmation = false;        clickWindow(bot, i)
                ...
                bot.currentWindow.requiresConfirmation = false;        clickWindow(bot, slotToClick)
            }
            ...
            if (title.toString().includes('Manage Auctions')) {
                ...
                for (let i = 0; i < window.slots.length; i++) {
                    ...
                    if (item && item.name === 'cauldron' && (item.nbt as any).value?.display?.value?.Name?.value?.toString().includes('Claim All')) {
                        ...
                        bot.currentWindow.requiresConfirmation = false;        clickWindow(bot, item.slot)
                        clearTimeout(timeout)
                        bot.removeAllListeners('windowOpen')
                        bot.state = null
                        resolve(true)
...
function claimExpiredAuction(bot, slot) {
    return new Promise(resolve => {
        ...
        bot.currentWindow.requiresConfirmation = false;        clickWindow(bot, slot)
    })
}
