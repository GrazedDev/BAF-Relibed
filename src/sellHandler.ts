...
async function sellHandler(data: SellData, bot: MyBot, sellWindow, ws: WebSocket, removeEventListenerCallback: Function) {
    ...
    if (title.toString().includes('Auction House')) {
        bot.currentWindow.requiresConfirmation = false;        clickWindow(bot, 15)
    }
    ...
    if (title == 'Create BIN Auction') {
        if (!setPrice && !durationSet) {
            if (!sellWindow.slots[13].nbt.value.display.value.Name.value.includes('Click an item in your inventory!')) {
                bot.currentWindow.requiresConfirmation = false;        clickWindow(bot, 13)
            }
            ...
            bot.currentWindow.requiresConfirmation = false;        clickWindow(bot, itemSlot)
            ...
            log('opening pricer')
            bot.currentWindow.requiresConfirmation = false;        clickWindow(bot, 31)
            setPrice = true
        } else if (setPrice && !durationSet) {
            bot.currentWindow.requiresConfirmation = false;        clickWindow(bot, 33)
        } else if (setPrice && durationSet) {
            const resetAndTakeOutItem = () => {
                bot.currentWindow.requiresConfirmation = false;        clickWindow(bot, 13) // Take the item out of the window
                removeEventListenerCallback()
                setPrice = false
                durationSet = false
                bot.state = null
            ...
            bot.currentWindow.requiresConfirmation = false;        clickWindow(bot, 29)
        }
    }
    if (title == 'Auction Duration') {
        ...
        bot.currentWindow.requiresConfirmation = false;        clickWindow(bot, 16)
    }
